...
const RUNE_ART_MASKS = webpackImages({
	Any:    require("./charge-templates/Any.mask.data.png"),
	Air:    require("./charge-templates/Air.mask.data.png"),
	Water:  require("./charge-templates/Water.mask.data.png"),
	Earth:  require("./charge-templates/Earth.mask.data.png"),
	Fire:   require("./charge-templates/Fire.mask.data.png"),
	Mind:   require("./charge-templates/Mind.mask.data.png"),
	...
	Law:    require("./charge-templates/Law.mask.data.png"),
	Nature: require("./charge-templates/Nature.mask.data.png"),
	Blood:  require("./charge-templates/Blood.mask.data.png"),
	Astral: require("./charge-templates/Astral.mask.data.png"),
	Soul:   require("./charge-templates/Soul.mask.data.png"),
	Time:   require("./charge-templates/Time.mask.data.png"),
});

// ── Full rune-art masks (25×25, composite of template + charge-0 fill) ───────
// Generated by scripts/generate-charge-templates.js. Covers the entire 25×25
// inner icon area including the digit rectangle, for visual debug display.
const RUNE_FULL_MASKS = webpackImages({
	Any:    require("./charge-templates/Any.fullmask.data.png"),
	Air:    require("./charge-templates/Air.fullmask.data.png"),
	Water:  require("./charge-templates/Water.fullmask.data.png"),
	Earth:  require("./charge-templates/Earth.fullmask.data.png"),
	Fire:   require("./charge-templates/Fire.fullmask.data.png"),
	Mind:   require("./charge-templates/Mind.fullmask.data.png"),
	Body:   require("./charge-templates/Body.fullmask.data.png"),
	Chaos:  require("./charge-templates/Chaos.fullmask.data.png"),
	Death:  require("./charge-templates/Death.fullmask.data.png"),
	Cosmic: require("./charge-templates/Cosmic.fullmask.data.png"),
	Law:    require("./charge-templates/Law.fullmask.data.png"),
	Nature: require("./charge-templates/Nature.fullmask.data.png"),
	Blood:  require("./charge-templates/Blood.fullmask.data.png"),
	Astral: require("./charge-templates/Astral.fullmask.data.png"),
	Soul:   require("./charge-templates/Soul.fullmask.data.png"),
	Time:   require("./charge-templates/Time.fullmask.data.png"),
});

// ── Combined (rune+charge) templates (25×25, full inner icon) ────────────────
// Generated by scripts/generate-combined-templates.js.  Keyed as "Rune_Charge",
// e.g. "Death_2" or "Death_25".  Digit overlay is INCLUDED (unlike rune templates
// which blank the digit area), so matching against a live frame simultaneously
// confirms the rune and the exact charge.
// eslint-disable-next-line @typescript-eslint/no-var-requires
const COMBINED_CTX = (require as any).context('./combined-templates', false, /\.data\.png$/);
const COMBINED_TEMPLATES: Record<string, ImageData> = {};
(COMBINED_CTX.keys() as string[]).forEach((k: string) => {
	const name = k.replace(/^\.\//, '').replace(/\.data\.png$/, '');
	COMBINED_TEMPLATES[name] = COMBINED_CTX(k) as ImageData;
});
...
const RUNE_COLORS: Record<string, string> = {
	Air:    "#b4b8bc",  // silver-grey
	Water:  "#44aacc",  // cyan-blue
	Earth:  "#9e7c38",  // warm brown
	Fire:   "#ee4422",  // orange-red
	Mind:   "#bb7700",  // amber-brown
	Body:   "#2233bb",  // deep navy
	Chaos:  "#dd8800",  // golden-orange
	Death:  "#686c70",  // slate grey
	Cosmic: "#d4cc00",  // bright yellow
	Law:    "#3366ff",  // royal blue
	Nature: "#22aa44",  // green
	Blood:  "#bb1100",  // blood red
	Astral: "#cc88bb",  // dusty lavender-pink
	Soul:   "#8855ee",  // blue-violet
	Time:   "#c8a822",  // warm amber-gold
	Any:    "#fbbf24",  // gold
};

const RUNE_RC_LEVEL: Partial<Record<string, number>> = {
	Air: 1, Mind: 1, Water: 5, Earth: 9, Fire: 14, Body: 20,
	Cosmic: 27, Chaos: 35, Astral: 40, Nature: 44, Law: 54,
	Death: 65, Blood: 77, Soul: 90, Time: 100,
};
...
let startupDetectionDone = false;
...
// Per-channel max color difference allowed for a combined template pixel to count as matched.
const COMBINED_COLOR_THRESH = 35;
const COMBINED_SCORE_THRESH = 0.80;

function readCombined(buff: any, runeName: string): { charge: number; score: number } | null {
	...
	for (const [key, tmpl] of Object.entries(COMBINED_TEMPLATES)) {
		if (!key.startsWith(`${runeName}_`)) continue;
		const charge = parseInt(key.slice(runeName.length + 1), 10);
		if (isNaN(charge)) continue;

		let tested = 0, passed = 0;
		for (let y = 0; y < tmpl.height; y++) {
			for (let x = 0; x < tmpl.width; x++) {
				const ti = (y * tmpl.width + x) * 4;
				if (tmpl.data[ti + 3] === 0) continue;
				tested++;
				// Live pixel at inner 25×25 (offset by 1 to skip 1px green border)
				const si = ((by + 1 + y) * buf.width + (bx + 1 + x)) * 4;
				const dR = Math.abs(buf.data[si]     - tmpl.data[ti]);
				const dG = Math.abs(buf.data[si + 1] - tmpl.data[ti + 1]);
				const dB = Math.abs(buf.data[si + 2] - tmpl.data[ti + 2]);
				if (Math.max(dR, dG, dB) <= COMBINED_COLOR_THRESH) passed++;
			}
		}
	...
	if (bestCharge >= 1 && bestScore >= COMBINED_SCORE_THRESH) {
		return { charge: bestCharge, score: bestScore };
	}
	return null;
}
...
function recordReading(runeName: string, chargeLevel: number): void {
	...
	saveData();
	updateChargeDisplay();
	setStatus(`Recorded: ${runeName} at charge ${chargeLevel}`);
...
function updateBuffPreviews(): void {
	...
	const matched = lastAllIcons.find(b => b.matched);
	if (!matched) return;

	const canvas = document.createElement("canvas");
	canvas.width  = 27;
	canvas.height = 27;
	canvas.getContext("2d")!.putImageData(matched.icon, 0, 0);
	canvas.className = "buff-thumb buff-thumb--matched";
...
function startPolling(): void {
	...
	setInterval(() => {
		try {
			...
			// Try combined (rune+charge) template first; fall back to charge-only readCharge().
			const combined  = readCombined(matchedBuff, runeName);
			const chargeOcr = readCharge(matchedBuff, runeName);
			const charge    = combined ? combined.charge : chargeOcr;
			if (combined && combined.charge !== chargeOcr) {
				addDebug(`Combined: ${runeName}_${combined.charge} (${combined.score.toFixed(3)}) overrides readCharge=${chargeOcr ?? "?"}`);
			}
			...
			if (pendingCount === STABLE_POLLS && !startupDetectionDone) {
				// First stable detection after startup — display but never record.
				startupDetectionDone = true;
				setStatus(`Detected: ${runeName} at charge ${charge ?? "?"}`);
				addDebug(`Startup detection — not recording`);
				if (runeName !== lastSeenRune || charge !== lastSeenCharge) {
				...
				const selStartup = document.getElementById("manual-rune-select") as HTMLSelectElement | null;
				if (selStartup && !selStartup.value) selStartup.value = runeName;
			} else if (pendingCount === STABLE_POLLS && (runeName !== lastSeenRune || charge !== lastSeenCharge)) {
...
function updateDebugCanvas(buff: any, runeName: string): void {
			...
			for (let tx = 0; tx < TMPL_W; tx++) {
				...
				if (tmpl) {
					...
					if (tA !== 255) {
						...
						// Tested pixel — pass or fail. Colored symbol pixels (saturated)
						// highlight in the rune's own hue; grey frame pixels use green/red.
						const tR = tmpl.data[ti], tG = tmpl.data[ti+1], tB = tmpl.data[ti+2];
						const coldif = Math.abs(iR-tR) + Math.abs(iG-tG) + Math.abs(iB-tB);
						const sat = Math.max(tR, tG, tB) - Math.min(tR, tG, tB);
						if (coldif <= 35) {
							if (sat > 28) {
								const bR = Math.min(255, Math.round(tR * 1.5 + 40));
								const bG = Math.min(255, Math.round(tG * 1.5 + 40));
								const bB = Math.min(255, Math.round(tB * 1.5 + 40));
	...
	// ── Full rune-art mask (25×25, composite template + charge-0 fill) ──────────
	const fullMask = RUNE_FULL_MASKS.loaded
		? ((RUNE_FULL_MASKS as any)[runeName] as ImageData | undefined) ?? null
		: null;

	const fullMaskCanvas = document.getElementById("debug-full-mask") as HTMLCanvasElement | null;
	if (fullMaskCanvas) {
		const ctx = fullMaskCanvas.getContext("2d")!;
		ctx.clearRect(0, 0, fullMaskCanvas.width, fullMaskCanvas.height);
		if (fullMask) {
			for (let fy = 0; fy < 25; fy++) {
				for (let fx = 0; fx < 25; fx++) {
					const fi = (fy * 25 + fx) * 4;
					const fA = fullMask.data[fi + 3];
					if (fA === 0) {
						ctx.fillStyle = "#1a1a2e"; // transparent pixel — show bg color
					} else {
						ctx.fillStyle = `rgb(${fullMask.data[fi]},${fullMask.data[fi+1]},${fullMask.data[fi+2]})`;
					}
					ctx.fillRect(fx * SCALE_TMPL, fy * SCALE_TMPL, SCALE_TMPL, SCALE_TMPL);
				}
			}
		} else {
			ctx.fillStyle = "#333";
	...
	// ── Compact preview canvases (shown inline next to the detected buff) ──────
	// Canvas 1 — rune: 3 × 54px tiles = [icon | rune template | rune diff]
	// Canvas 2 — charge: 3 × 54px tiles = [binarized | charge template | charge diff]
	const PT = 54; // tile size (matches buff-thumb CSS height)
	const PS = 2;  // pixel scale: 27×2=54 for icon, 25×2=50 (+2px margin) for templates

	const runePreview = document.getElementById("preview-debug-rune") as HTMLCanvasElement | null;
	if (runePreview) {
		const ctx = runePreview.getContext("2d")!;
		ctx.fillStyle = "#0a0a12";
		ctx.fillRect(0, 0, runePreview.width, runePreview.height);

		// Tile separators
		ctx.fillStyle = "rgba(255,255,255,0.06)";
		ctx.fillRect(PT, 0, 1, PT);
		ctx.fillRect(PT * 2, 0, 1, PT);

		// Tile 0: raw icon at 2×
		for (let y = 0; y < 27; y++) {
			for (let x = 0; x < 27; x++) {
				const si = ((by + y) * buf.width + (bx + x)) * 4;
				ctx.fillStyle = `rgb(${buf.data[si]},${buf.data[si+1]},${buf.data[si+2]})`;
		...
		// Charge template for overlay (hoisted outside loops)
		const cTmpl = lastBestCharge >= 1
			? (CHARGE_TEMPLATES as any)[`n${lastBestCharge}`] as ImageData | null
			: null;

		// Tile 1: rune template + charge pixels in the transparent digit zone.
		// Template rows 11–24 map to charge rows 0–13 (icon rows 12–25).
		// Opaque template pixels show the rune art color; transparent pixels in the
		// charge zone show the binarized charge pixel instead of dimmed icon.
		if (tmpl) {
			for (let ty = 0; ty < TMPL_H; ty++) {
				for (let tx = 0; tx < TMPL_W; tx++) {
					const iconSi = ((by + 1 + ty) * buf.width + (bx + 1 + tx)) * 4;
					...
					let color: string;
					if (tA === 255) {
						color = `rgb(${tmpl.data[ti]},${tmpl.data[ti+1]},${tmpl.data[ti+2]})`;
					} else if (ty >= 11) {
						const ci = (ty - 11) * CMW + tx;
						if      (artMaskedPx[ci])      color = "#ff8800";
						else if (chargeMini[ci] === 1) color = "#86efac";
						else if (chargeMini[ci] === 2) color = "#3a5a45";
						else                           color = "#1e1e2a";
					} else {
						color = `rgba(${iR},${iG},${iB},0.3)`;
					}
					ctx.fillStyle = color;
					ctx.fillRect(PT + 2 + tx * PS, 2 + ty * PS, PS, PS);
				}
			}
		}

		// Tile 2: rune diff + charge diff in the transparent digit zone.
		// Opaque template pixels: brightened template color for passes (no yellow-skip
		// so white symbols like Death remain visible); red for fails.
		// Transparent pixels in charge zone: charge diff (green/red/teal/orange).
		if (tmpl) {
			for (let ty = 0; ty < TMPL_H; ty++) {
				for (let tx = 0; tx < TMPL_W; tx++) {
					const iconSi = ((by + 1 + ty) * buf.width + (bx + 1 + tx)) * 4;
					...
					if (tA === 255) {
						const tR = tmpl.data[ti], tG = tmpl.data[ti+1], tB = tmpl.data[ti+2];
						// White/black icon pixels count as passing when the template also
						// expects bright/dark (e.g. Death's white symbol) — skip yellow.
						const iconSkip = (iR === 255 && iG === 255 && iB === 255) || (iR === 0 && iG === 0 && iB === 0);
						const passes   = iconSkip || Math.abs(iR-tR) + Math.abs(iG-tG) + Math.abs(iB-tB) <= 35;
						if (passes) {
							const sat  = Math.max(tR, tG, tB) - Math.min(tR, tG, tB);
							const luma = Math.max(tR, tG, tB);
							// Colored symbol (saturated) OR bright symbol like Death's white skull
							// → show in the rune's own color. Medium/dark grey → green (frame).
							if (sat > 28 || luma > 200) {
								const boost = sat > 28 ? 1.5 : 1.4;
								const add   = sat > 28 ? 40  : 25;
								const bR = Math.min(255, Math.round(tR * boost + add));
								const bG = Math.min(255, Math.round(tG * boost + add));
								const bB = Math.min(255, Math.round(tB * boost + add));
								color = `rgb(${bR},${bG},${bB})`;
							} else {
						...
						const ci    = (ty - 11) * CMW + tx;
						const alpha = cTmpl ? cTmpl.data[ci * 4 + 3] : 0;
						const mv    = chargeMini[ci];
						if      (alpha === 255) color = mv === 1 ? "#86efac"             : "rgba(220,40,40,0.9)";
						else if (alpha === 128) color = mv === 2 ? "rgba(0,200,200,0.9)" : "rgba(255,140,0,0.9)";
						else                   color = mv === 1 ? "#3a5a45" : "#1e1e2a";
					} else {
						color = `rgba(${iR},${iG},${iB},0.3)`;
					}
					ctx.fillStyle = color;
					ctx.fillRect(PT * 2 + 2 + tx * PS, 2 + ty * PS, PS, PS);
				}
			}
			// Charge label rendered in the div below the canvas (avoids pixel overlap)
			const chargeLabel = document.getElementById("preview-charge-label");
			if (chargeLabel) {
				if (lastBestCharge >= 1) {
					chargeLabel.textContent = `charge ${lastBestCharge} · ${(lastBestChargeScore * 100).toFixed(0)}%`;
					(chargeLabel as HTMLElement).style.color = lastBestChargeScore >= 0.65 ? "#fbbf24" : "#f87171";
				} else {
...
function updateStats(): void {
	...
	if (!el) return;
	el.textContent = `${data.totalReadings} readings`;
}
...
function renderHeatmap(): void {
	...
	for (let ri = 0; ri < runes.length; ri++) {
		const rune  = runes[ri];
		const y     = MT + ri * CH;
		const hex   = RUNE_COLORS[rune] ?? "#c084fc";
		...
		for (let c = 0; c <= 49; c++) {
			...
			const colMax = tot > 0 ? Math.max(...Object.values(lvl)) : 0;
			const x      = ML + c * CW;
			ctx.fillStyle = CHOICE_LEVELS.has(c) ? "#0e2a45" : "#10101e";
			ctx.fillRect(x, y, CW - 1, CH - 1);
			if (count > 0 && colMax > 0) {
				const alpha = (count / colMax) * (tot < 3 ? 0.4 : 1.0);
...
function computeRuneTotals(): [string, number][] {
	const totals: Record<string, number> = {};
	for (const lvl of Object.values(data.counts))
		for (const [rune, count] of Object.entries(lvl))
			totals[rune] = (totals[rune] ?? 0) + count;
	return Object.entries(totals).sort((a, b) => b[1] - a[1]);
}

function renderRuneTotals(): void {
	const entries = computeRuneTotals();
	if (!entries.length) return;

	const grandTotal = entries.reduce((s, [, c]) => s + c, 0);
	const maxCount   = entries[0][1];

	const LV_W = 24, ML = 78, MR = 54, MT = 6, MB = 6;
	const BAR_H = 13, GAP = 3, ROW_H = BAR_H + GAP;
	const CHART_W = 270;
	const W = ML + CHART_W + MR;
	const H = MT + entries.length * ROW_H - GAP + MB;

	const ctx = getVizCtx("totals-bar-container", W, H);
	if (!ctx) return;
	...
	for (let i = 0; i < entries.length; i++) {
		const [rune, count] = entries[i];
		const y     = MT + i * ROW_H;
		const color = RUNE_COLORS[rune] ?? "#c084fc";
		const barW  = Math.max(1, Math.round((count / maxCount) * CHART_W));
		const pct   = Math.round((count / grandTotal) * 100);

		const lvl = RUNE_RC_LEVEL[rune];
		if (lvl !== undefined) {
			ctx.font = "8px monospace";
			ctx.textAlign = "right";
			ctx.fillStyle = "#4a5568";
			ctx.fillText(`${lvl}`, LV_W, y + BAR_H - 2);
		}

		ctx.font = "bold 10px sans-serif";
		ctx.textAlign = "right";
		ctx.fillStyle = color;
		ctx.fillText(rune, ML - 4, y + BAR_H - 2);

		ctx.fillStyle = "#10101e";
		ctx.fillRect(ML, y, CHART_W, BAR_H);

		ctx.fillStyle = color;
		ctx.globalAlpha = 0.7;
		ctx.fillRect(ML, y, barW, BAR_H);
		ctx.globalAlpha = 1;

		ctx.font = "10px monospace";
		ctx.textAlign = "left";
		ctx.fillStyle = "#94a3b8";
		ctx.fillText(`${count}  ${pct}%`, ML + CHART_W + 5, y + BAR_H - 2);
	}
}

function renderRuneDonut(): void {
	const entries = computeRuneTotals();
	if (!entries.length) return;

	const grandTotal = entries.reduce((s, [, c]) => s + c, 0);
	const W = 378, H = 200;
	const ctx = getVizCtx("totals-donut-container", W, H);
	if (!ctx) return;

	ctx.fillStyle = "#1a1a2e";
	ctx.fillRect(0, 0, W, H);

	const CX = 100, CY = 100, R_OUT = 88, R_IN = 52;
	let angle = -Math.PI / 2;

	for (const [rune, count] of entries) {
		const sweep = (count / grandTotal) * 2 * Math.PI;
		const color = RUNE_COLORS[rune] ?? "#c084fc";
		ctx.beginPath();
		ctx.moveTo(CX, CY);
		ctx.arc(CX, CY, R_OUT, angle, angle + sweep);
		ctx.closePath();
		ctx.fillStyle = color;
	...
	ctx.fillText(String(grandTotal), CX, CY + 5);
	ctx.fillStyle = "#64748b";
	ctx.font = "9px sans-serif";
	ctx.fillText("readings", CX, CY + 17);

	// legend — two columns of 8, one line per entry
	const LX1 = 198, LX2 = 288, LY0 = 10, ROW = 22;
	const COL_W = 84; // width of each column for right-aligning pct
	for (let i = 0; i < entries.length; i++) {
		const [rune, count] = entries[i];
		const col = i < 8 ? 0 : 1;
		const row = i < 8 ? i : i - 8;
		const lx  = col === 0 ? LX1 : LX2;
		const ly  = LY0 + row * ROW;
		const pct = Math.round((count / grandTotal) * 100);
		const color = RUNE_COLORS[rune] ?? "#c084fc";
		...
		ctx.textAlign = "left";
		ctx.fillStyle = "#94a3b8";
		ctx.fillText(rune, lx + 12, ly + 8);

		ctx.font = "9px monospace";
		ctx.textAlign = "right";
		ctx.fillStyle = "#64748b";
		ctx.fillText(`${count} ${pct}%`, lx + COL_W, ly + 8);
	}
}

function renderRuneTreemap(): void {
	const entries = computeRuneTotals();
	if (!entries.length) return;

	const grandTotal = entries.reduce((s, [, c]) => s + c, 0);
	const W = 378, H = 80;
	const ctx = getVizCtx("totals-treemap-container", W, H);
	if (!ctx) return;
	...
	let x = 0;
	for (const [rune, count] of entries) {
		const tileW = Math.round((count / grandTotal) * W);
		if (tileW < 1) continue;
		const color = RUNE_COLORS[rune] ?? "#c084fc";

		ctx.fillStyle = color;
		ctx.globalAlpha = 0.78;
		ctx.fillRect(x, 0, tileW - 1, H);
		ctx.globalAlpha = 1;

		if (tileW >= 14) {
			const label = rune.length > 5 ? rune.slice(0, 4) + "." : rune;
			ctx.fillStyle = "#ffffff";
			ctx.font = "bold 9px sans-serif";
			ctx.textAlign = "center";
			ctx.fillText(label, x + (tileW - 1) / 2, H / 2 - 3);
			if (tileW >= 22) {
				ctx.font = "9px monospace";
				ctx.fillText(String(count), x + (tileW - 1) / 2, H / 2 + 9);
			}
		}
		x += tileW;
	}
...
function init(): void {
	...
	document.getElementById("import-file")?.addEventListener("change", (e) => {
		...
		reader.onload = (ev) => {
			try {
				const parsed = JSON.parse(ev.target?.result as string) as FrequencyData;
				if (!parsed.counts || typeof parsed.totalReadings !== "number") {
					addDebug("Import failed: invalid format");
					return;
				}
				data = parsed;
				saveData();
				updateChargeDisplay();
				renderAll();
				setStatus(`Imported ${parsed.totalReadings} readings from ${file.name}`);
				addDebug(`Imported ${parsed.totalReadings} readings from ${file.name}`);
			} catch (err) {
				addDebug(`Import failed: ${String(err)}`);
			}
